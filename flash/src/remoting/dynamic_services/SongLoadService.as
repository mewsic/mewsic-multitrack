package remoting.dynamic_services {
	 ...
	 * @since Aug 06, 2008
	 */
	public class SongLoadService extends ServiceCommon implements IService {
		 ...
		 * Constructor.
		 */
		public function SongLoadService() {
			super();
			
			$serviceID = sprintf('songLoad.%u.%u', uint(new Date()), Rnd.integer(1000, 9999));
		...
		/**
		 * Dump song load results.
		 * @return Song load dump
		 */
		...
		private function _onResponse():void {
			try {
				...
				if(Settings.isServiceDumpEnabled) Logger.debug(sprintf('Service %s: Song load info dump:\n%s', $serviceID, _songData.toString()));
