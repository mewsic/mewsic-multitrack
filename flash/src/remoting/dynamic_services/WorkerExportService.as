package remoting.dynamic_services {
	 ...
	 * @since Jul 24, 2008
	 */
	public class WorkerExportService extends ServiceCommon implements IService {
		 ...
		 * Constructor.
		 */
		public function WorkerExportService() {
			super();
			
			$serviceID = sprintf('exportWorker.%u.%u', uint(new Date()), Rnd.integer(1000, 9999));
		...
		private function _onResponse():void {
			try {
				...
				if(Settings.isServiceDumpEnabled) Logger.debug(sprintf('Service %s: Export worker dump:\n%s', $serviceID, _statusData.toString()));
				...
				dispatchEvent(new RemotingEvent(RemotingEvent.REQUEST_FAILED, false, false, sprintf('Service %s: Could not parse export worker data.\n%s', $serviceID, err.message)));
			}
		}
		...
		private function _onError():void {
			dispatchEvent(new RemotingEvent(RemotingEvent.REQUEST_FAILED, false, false, sprintf('Service %s: Could not load export worker data.', $serviceID)));
		}
	}
}
