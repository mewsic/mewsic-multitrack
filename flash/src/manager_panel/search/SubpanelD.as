package manager_panel.search {
	 ...
	 * @since Jul 12, 2008
	 */
	public class SubpanelD extends SubpanelCommon {
		 ...
		 */
		public function SubpanelD(height:uint, c:Object = null) {
			super(height, c);
		...
		public function set currentType(t:String):void {
			Logger.debug(sprintf('Switchning subpanel D search mode to %s', t));
		...
		public function fill():void {
			if($isFilled) throw new Error('Subpanel D already filled. Clean it first.');
			else {
				var sy:uint = 0;
				try {
					...
					Logger.warn(sprintf('Error adding subpanel D row (%s)', err.message));
				}
		...
		public function clean():void {
			Logger.debug('Cleaning current subpanel D rows');
			for each(var row:* in _rowList) {
				try {
					...
					Logger.error(sprintf('Error cleaning subpanel D row (%s)', err.message));
				}
			}
			_rowList = new Array();
