package controls {
	...
	public class Input extends MorphSprite {
		...
		protected var $isDropbox:Boolean;
		...
		public function Input(c:Object = null) {
			...
			if(c.introText != undefined) {
				_isIntroEnabled = c.introText;
				_introText = c.introText;
			}
			...
			$textPressTF = new QTextField(assign(textData, {visible:!_isIntroEnabled, defaultTextFormat:(c.textPressFormat != undefined) ? c.textPressFormat : defTextPressFormat, filters:(c.textPressFilters != undefined) ? c.textPressFilters : defTextPressFilters, alpha:0}));
			$textIntroTF = new QTextField({visible:_isIntroEnabled, defaultTextFormat:(c.textIntroFormat != undefined) ? c.textIntroFormat : defTextIntroFormat, filters:(c.textIntroFilters != undefined) ? c.textIntroFilters : defTextIntroFilters, alpha:(c.textIntroAlpha != undefined) ? c.textIntroAlpha : defTextIntroAlpha, mouseEnabled:false, multiline:false, text:_introText});
			...
			$textPressTF.type = ($isDropbox) ? TextFieldType.DYNAMIC : TextFieldType.INPUT;
			$textPressTF.selectable = (!$isDropbox);
		...
		public function reset():void {
			...
			$textOverTF.text = '';
			$textPressTF.text = '';
			$textIntroTF.text = _introText;
			
			$textOutTF.visible = !_isIntroEnabled;
			$textOverTF.visible = !_isIntroEnabled;
			$textPressTF.visible = !_isIntroEnabled;
			$textIntroTF.visible = _isIntroEnabled;
		}
		...
		public function set text(value:String):void {
			...
			if($isDropbox) {
				...
				$textOutTF.visible = $textPressTF.visible = true;
			}			
		...
		private function _onFocusOut(event:FocusEvent):void {
			...
			if(_isIntroEnabled && !$isDropbox && this.text == '') {
